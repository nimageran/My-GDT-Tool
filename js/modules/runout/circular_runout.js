...
function drawResultsCard() {
    const { history, toleranceRunout } = state;
    // FIM (full indicator movement) = highest − lowest reading
    const fim = history.length ? Math.max(...history) - Math.min(...history) : 0;
    const pass = fim <= toleranceRunout;
    svgContainer.appendChild(resultsCard({
        title: 'Circular runout', pass,
        rows: [['Dial movement (FIM)', f4(fim), { strong: true, color: pass ? COLORS.pass : COLORS.fail }], ['Allowed', f4(toleranceRunout)]],
