...
export function draw(canvas) {
    const g = el('g');
    g.appendChild(el('rect', { x: 150, y: 180, width: 700, height: 290,
        rx: 12, fill: '#f8fafc', stroke: '#cbd5e1', 'stroke-width': 1.5, 'stroke-dasharray': '8 6' }));
    g.appendChild(txt('ROADMAP #5', 190, 230, 12, '#94a3b8', true));
    g.appendChild(txt('ISO Fits Decoder (H7/g6)', 190, 265, 24, '#0f172a', true));
    g.appendChild(txt('Planned scope:', 190, 305, 14, '#64748b', true));
    g.appendChild(txt('\u2022  Input nominal size + fit class (e.g. 30 H7/g6)', 200, 335, 14, '#334155'));
    g.appendChild(txt('\u2022  Output: actual limit dimensions for hole and shaft', 200, 365, 14, '#334155'));
    g.appendChild(txt('\u2022  Clearance / interference range and fit character', 200, 395, 14, '#334155'));
    g.appendChild(txt('\u2022  Preview: shaft-in-hole tolerance zone diagram', 200, 425, 14, '#334155'));
...
export function loadControls(container) {
    container.innerHTML = `
      <div class="p-4 bg-slate-50 rounded border border-slate-200 text-sm text-slate-600 space-y-2">
        ...
        <p class="font-mono text-xs bg-white border border-slate-200 rounded p-2">Build fits.js per the spec.</p>
