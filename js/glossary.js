...
export const GLOSSARY = [
    ...
    { term: 'Lobing', match: ['lobing', 'lobed'],
      meaning: 'An out-of-round shape with bumps evenly spaced around it (often 3 or 5). A micrometer reads the same size in every direction, so it can hide the error.',
      see: tool('INSPECTION', 'methods', 'Measurement Methods') },
    ...
    { term: 'Functional gauge', match: ['functional gauge', 'functional gauges', 'go gauge'],
      meaning: 'A fixture with pins at virtual condition, placed at true position on datum simulators. If the part drops on, the pattern passes; sizes are checked separately.',
      see: tool('INSPECTION', 'functional_gauge', 'Functional Gauge Designer') },
    ...
    { term: 'Break sharp edges', match: ['break sharp edges', 'edge break'],
      ...
      see: tool('DECODE', 'drawing_notes', 'Notes & Abbreviations') },
    { term: 'Best fit', match: ['best fit', 'best-fit'],
      meaning: 'A CMM alignment that moves the measured shape until it matches the model as closely as possible. Only right when the frame has no datums; otherwise it hides location errors.',
      see: tool('INSPECTION', 'methods', 'Measurement Methods') },
